...
Shares the same `"screener_all"` cache key as `quote()`, so calling both within the same
...
Returns a `pd.DataFrame` with one row per PSX-listed symbol (approximately 729 rows).
...
<ResponseField name="sector" type="float64" required>
  Raw numeric sector code as returned by PSX — not enriched to a sector name. Join
  against `psxdata.symbols()` on `sector_name` if you need the sector label.
</ResponseField>

<ResponseField name="listed_in" type="object" required>
...
<ResponseField name="free_float" type="float64" required>
  Free-float shares as a percentage of total shares outstanding.
</ResponseField>

<ResponseField name="volume_avg_30d" type="float64" required>
  30-day average trading volume.
</ResponseField>

<ResponseField name="change_1y_pct" type="float64" required>
